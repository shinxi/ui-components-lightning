...
import { STANDARD_ICONS, CUSTOM_ICONS, ACTION_ICONS, DOCTYPE_ICONS, UTILITY_ICONS } from './IconConstant';
...
class Icon extends Component {
  ...
  getIconColor(fillColor, category, icon) {
    ...
    let setIconColor = null;

    if (category === 'custom') {
      setIconColor = icon.replace(/^custom/, 'custom-');
    } else if (category === 'action') {
      setIconColor = icon.replace(/^new_custom/, 'custom-');
    } else if (fillColor) {
      setIconColor = fillColor;
    } else if (category === 'doctype' || category === 'utility') {
      setIconColor = null;
    } else {
      setIconColor = `${category}-${(icon || '').replace(/_/g, '-')}`;
    }
    return (this.state.setIconColor ? this.state.iconColor : setIconColor);
  }
