...
// This API will be changed to createPortal after React 16
...
class Popover extends React.Component {
  ...
  componentDidMount() {
    const portalId = `Popover__portal_${this.id}`;
  ...
  renderPortal = props => {
    const { position, theme, isTooltip, content } = props;
    ...
    if (typeof content === 'string') {
      popoverContent = <PopoverBody>{content}</PopoverBody>;
    } else {
  ...
  render() {
    const { children, position, theme, isTooltip, content, ...props } = this.props;
    delete props.defaultVisible;
    return (
      <div id={`${Popover}__${this.id}`} className="Popover" {...props}>
        <div
